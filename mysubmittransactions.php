<?php
		...
		$query1="SELECT id,bookid ,state,executorid FROM transaction where commandid='$userid'";
		...
		while($row = mysqli_fetch_array($data))
		{
			...
			if($row['executorid']){
			$query3="SELECT username FROM users WHERE userid = ".$row['executorid'];
